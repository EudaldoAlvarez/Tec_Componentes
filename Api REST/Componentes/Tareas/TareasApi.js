 ...
 */
Tareas.post("/Consultar", Middleware, async (req, res) => {
    let { email } = req.body;
    const { status, response } = await Services.consultar(email);
 ...
 */
Tareas.post("/Editar", Middleware,async (req, res) => {
    let { id, titulo, descripcion, fecha_inicio, fecha_fin } = req.body;
    const { status, response } = await Services.Editar(id, titulo, descripcion, fecha_inicio, fecha_fin);
    res.status(status).json(response);
});
...
Tareas.post("/Crear",Middleware, async (req, res) => {
    ...
    const { status, response } = await Services.Crear(email, titulo, descripcion, fecha_inicio, fecha_fin);
    res.status(status).json(response);
});
 ...
 */
 Tareas.post("/Borrar", Middleware, async (req, res) => {
    let { id } = req.body;
    const { status, response } = await Services.Borrar(id);
    res.status(status).json(response);
});
